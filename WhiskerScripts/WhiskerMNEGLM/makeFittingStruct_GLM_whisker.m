function gg = makeFittingStruct_GLM_whisker(sta,DTsim,ReducedSpace,glmstruct,cellnumToFit);
...
kbasprs.neye = 0 ; % 5 ; % Number of "identity" basis vectors near time of spike;
...
if (nargin >= 4) 
    ...
    if isfield(glmstruct, 'ihbasprs')
        if ~isempty(glmstruct.ihbasprs);
            ...
            if size(glmstruct.ih,2)>1 && (nargin < 5)
